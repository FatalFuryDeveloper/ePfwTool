...
(function(angular) {
    ...
    function GestorFaseControlador($rootScope, $scope, $uibModal, $state, $translate, $timeout,
        ...
        vm.camposTablas     = TablaFactory.tablaFase();
        ...
        vm.expRegularTexto  = /^[a-zA-ZÁáÀàÉéÈèÍíÌìÓóÒòÚúÙùÑñüÜ0-9! #()_\-\.,~\*\?_~\/]{0,50}$/;
        vm.expRegularFinal  = "";
        vm.alerts           = [];
        vm.guardar          = true;
        ...
        vm.rutaPHP          = vm.paths.rcs.php.ruta + vm.paths.rcs.php.admTablaFases;
        vm.init             = inicializar;
        ...
        function actualizarOrden (oldIndex, newIndex){
            ...
            for(var x = 1; x  <= maximo; x++){
                vm.id    = vm.datosTabla[x-1].ctf_id;
                vm.orden = contador;
                contador++;
                if(x === oldIndex){
                    vm.orden = newIndex;
                ...
                //console.log("vm.id: "+vm.id + ", vm.orden: "+ vm.orden);
         ...
         */
        vm.validar = function (){
            if (vm.txtFase !== "" && vm.txtDescripcion !== "" && vm.cmbEstado.length > 0 ) {
        ...
        function tablaAdministracion(){
            ...
            vm.dtAdministrarColumns = TablaFactory.contruirColumnasTablaFa(vm.camposTablas, "fase");
        }
        ...
        function transferirMensaje() {
           var mensaje = i18nFactory.i18nTraduccion(vm.lbl.msj.mod.confirmarfase) + vm.fase + "  ?";
           return mensaje;
        }
