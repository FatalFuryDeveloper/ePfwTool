<?php
    ...
    function consultar(){
		...
		$resultado= mysql_query("SELECT * FROM catalogo_nivel WHERE ctn_eliminado='$eliminado'");
	...
	function consultarNivel(){
		global $conexion, $data;
		$id 		= $data["id"];
		$resultado  = mysql_query("SELECT * FROM catalogo_nivel WHERE ctn_id='$id'");
		codificarJSON($resultado);
	}
	...
	function insertar(){
		...
		$estado		= $data["estado"];
		$resultado  = mysql_query("INSERT INTO catalogo_nivel (ctn_nombre, ctn_descripcion, ctn_estado, ctn_eliminado) VALUES ('$nombre', '$descripcion', '$estado','false')  ");
		validarError();
	}
	...
	function modificar(){
		...
		$descripcion= $data["descripcion"];
		$estado		= $data["estado"];
		$resultado 	= mysql_query("UPDATE catalogo_nivel SET ctn_nombre='$nombre', ctn_descripcion='$descripcion', ctn_estado='$estado'  WHERE ctn_id='$id'");
		validarError();
	}
	...
	function cambiarEstado(){
		...
		$id 		= $data["id"];
		$estado		= $data["estado"];
		$resultado 	= mysql_query("UPDATE catalogo_nivel SET ctn_estado='$estado' WHERE ctn_id='$id'");
		validarError();
	}
	...
	function eliminar(){
		...
		$id 		= $data["id"];
		$eliminado 	= $data["eliminado"];
		$resultado 	= mysql_query("UPDATE catalogo_nivel SET ctn_eliminado='$eliminado' WHERE ctn_id='$id'");
		validarError();
	}
